...
import { computeReshapedDimensions } from './../utils/index'
import { getMediaDimensions, imageToSquare, toNetInput } from './../dom'
import { Dimensions, Rect } from './../classes'
...
export class TinyYolov2Trainable extends TinyYolov2 {
  ...
  public async backward(
    ...
    const reshapedImgDims = computeReshapedDimensions(
      getMediaDimensions(img),
      inputSize
    )
    const filteredGroundTruthBoxes = this.filterGroundTruthBoxes(
      groundTruth,
      reshapedImgDims,
      minBoxSize
    )
  ...
  public computeLoss(
    ...
    return tf.tidy(() => {
      const lossFunction = new TinyYolov2LossFunction(
        outputTensor,
        groundTruth,
        predictedBoxes,
        reshapedImgDims,
  ...
  public filterGroundTruthBoxes(
    ...
    return groundTruth.filter(({ x, y, width, height }) => {
      const box = new Rect(x, y, width, height).rescale({
        height: imgHeight,
        width: imgWidth
      })
