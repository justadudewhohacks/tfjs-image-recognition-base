...
export function validateConfig(config: any) {
  ...
  if (typeof config.withSeparableConvs !== 'boolean') {
    throw new Error(
      `config.withSeparableConvs has to be a boolean, have: ${
        config.withSeparableConvs
      }`
    )
  }

  if (
    !isNumber(config.iouThreshold) ||
    config.iouThreshold < 0 ||
    config.iouThreshold > 1.0
  ) {
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      `config.classes has to be an array class names: string[], have: ${JSON.stringify(
        config.classes
      )}`
    )
  }
  ...
  if (
    ...
    !config.anchors
      .map((a: any) => a || {})
      .every((a: any) => isNumber(a.x) && isNumber(a.y))
  ) {
    throw new Error(
...
export function validateTrainConfig(config: any): TinyYolov2TrainableConfig {
  if (
    ...
    throw new Error(
      `for training you have to specify noObjectScale, objectScale, coordScale, classScale parameters in your config.json file`
    )
  }
