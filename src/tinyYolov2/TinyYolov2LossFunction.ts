...
import { BoundingBox, Dimensions, Rect, Point } from './../classes'
...
export class TinyYolov2LossFunction {
  ...
  constructor(
    ...
    this._config = config
    this._reshapedImgDims = new Dimensions(
      reshapedImgDims.width,
      reshapedImgDims.height
    )
    ...
    const {
      ...
      coordBoxSizeMask,
      scoreMask
    } = this.createCoordAndScoreMasks()

    this.noObjectLossMask = tf.tidy(() =>
      tf.mul(scoreMask, tf.sub(tf.scalar(1), groundTruthMask))
    ) as tf.Tensor4D
    this.objectLossMask = tf.tidy(() =>
      tf.mul(scoreMask, groundTruthMask)
    ) as tf.Tensor4D
    this.coordBoxOffsetMask = tf.tidy(() =>
      tf.mul(coordBoxOffsetMask, groundTruthMask)
    ) as tf.Tensor4D
    this.coordBoxSizeMask = tf.tidy(() =>
      tf.mul(coordBoxSizeMask, groundTruthMask)
    ) as tf.Tensor4D

    const classScoresMask = tf.tidy(() =>
      tf.sub(
        tf.scalar(1),
        coordBoxOffsetMask.add(coordBoxSizeMask).add(scoreMask)
      )
    )
    this.groundTruthClassScoresMask = tf.tidy(() =>
      tf.mul(classScoresMask, groundTruthMask)
    ) as tf.Tensor4D
  }
  ...
  public computeLoss() {
    return tf.tidy(() => {
      ...
      const totalLoss: tf.Tensor<tf.Rank.R0> = tf.tidy(() =>
        noObjectLoss
          .add(objectLoss)
          .add(coordLoss)
          .add(classLoss)
  ...
  public computeObjectLoss(): tf.Tensor<tf.Rank.R0> {
    return tf.tidy(() =>
      this.computeLossTerm(
        ...
        tf.sub(
          this.toOutputTensorShape(this.computeIous()),
          tf.sigmoid(this.outputTensor)
        )
      )
    )
  ...
  public computeCoordLoss(): tf.Tensor<tf.Rank.R0> {
    return tf.tidy(() =>
      this.computeLossTerm(
        ...
        tf.scalar(1),
        tf.add(
          this.computeCoordBoxOffsetError(),
          this.computeCoordBoxSizeError()
        )
      )
    )
  }
  ...
  public computeCoordBoxOffsetError(): tf.Tensor4D {
    return tf.tidy(() => {
      ...
      const gtBoxOffsets = tf.mul(
        mask,
        this.toOutputTensorShape(this.computeCoordBoxOffsets())
      )
  ...
  public computeCoordBoxSizeError(): tf.Tensor4D {
    return tf.tidy(() => {
      ...
      const gtBoxSizes = tf.mul(
        mask,
        this.toOutputTensorShape(this.computeCoordBoxSizes())
      )
  ...
  private computeLossTerm(
    scale: number,
    mask: tf.Tensor<tf.Rank>,
    lossTensor: tf.Tensor4D
  ): tf.Tensor<tf.Rank.R0> {
    return tf.tidy(() =>
      tf.mul(tf.scalar(scale), this.squaredSumOverMask(mask, lossTensor))
    )
  }
  ...
  private validateGroundTruthBoxes(groundTruth: GroundTruth[]) {
    groundTruth.forEach(({ x, y, width, height, label }) => {
      if (
        typeof label !== 'number' ||
        label < 0 ||
        label > this.config.classes.length - 1
      ) {
        throw new Error(
          `invalid ground truth data, expected label to be a number in [0, ${this
      ...
      if (
        ...
        throw new Error(
          `invalid ground truth data, box is out of image boundaries ${JSON.stringify(
            { x, y, width, height }
          )}`
        )
      }
  ...
  private assignGroundTruthToAnchors(groundTruth: GroundTruth[]) {
    const groundTruthBoxes = groundTruth.map(
    ...
    return groundTruthBoxes.map(({ box, label }) => {
      const { left, top, bottom, right, x, y, width, height } = box.rescale(
        this.reshapedImgDims
      )
      ...
      const anchorsByIou = this.anchors
        .map((anchor, idx) => ({
          idx,
          iou: iou(
  ...
  private createCoordAndScoreMasks() {
    return tf.tidy(() => {
      const coordBoxOffsetMask = tf.zeros([
        this.numCells,
        this.numCells,
        this.numBoxes,
        this.boxEncodingSize
      ])
      const coordBoxSizeMask = tf.zeros([
        this.numCells,
        this.numCells,
        this.numBoxes,
        this.boxEncodingSize
      ])
      const scoreMask = tf.zeros([
        this.numCells,
        this.numCells,
        this.numBoxes,
        this.boxEncodingSize
  ...
  private computeIous() {
    const isSameAnchor = (p1: GridPosition) => (p2: GridPosition) =>
      p1.row === p2.row && p1.col === p2.col && p1.anchor === p2.anchor
    ...
    this.groundTruth.forEach(({ row, col, anchor, box }) => {
      const predBox = this.predictedBoxes.find(
        isSameAnchor({ row, col, anchor })
      )

      if (!predBox) {
        throw new Error(
          `no output box found for: row ${row}, col ${col}, anchor ${anchor}`
        )
      }
