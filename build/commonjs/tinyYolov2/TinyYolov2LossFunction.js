...
var TinyYolov2LossFunction = /** @class */ (function () {
    function TinyYolov2LossFunction(outputTensor, groundTruth, predictedBoxes, reshapedImgDims, config) {
        this._config = config;
        this._reshapedImgDims = new classes_1.Dimensions(reshapedImgDims.width, reshapedImgDims.height);
    ...
    TinyYolov2LossFunction.prototype.computeClassLoss = function () {
        ...
        return tf.tidy(function () {
            var classLossTensor = tf.tidy(function () {
                var predClassScores = tf.mul(tf.softmax(_this.outputTensor.reshape([
                    _this.numCells,
                    _this.numCells,
                    _this.numBoxes,
                    _this.boxEncodingSize
                ]), 3), _this.groundTruthClassScoresMask);
    ...
    TinyYolov2LossFunction.prototype.validateGroundTruthBoxes = function (groundTruth) {
        ...
        groundTruth.forEach(function (_a) {
            ...
            if (typeof label !== 'number' ||
                label < 0 ||
                label > _this.config.classes.length - 1) {
                throw new Error("invalid ground truth data, expected label to be a number in [0, " + (_this
    ...
    TinyYolov2LossFunction.prototype.assignGroundTruthToAnchors = function (groundTruth) {
        ...
        var groundTruthBoxes = groundTruth.map(function (_a) {
            var x = _a.x, y = _a.y, width = _a.width, height = _a.height, label = _a.label;
            return ({
                box: new classes_1.Rect(x, y, width, height),
                label: label
            });
        });
        return groundTruthBoxes.map(function (_a) {
            ...
            var anchorsByIou = _this.anchors
                .map(function (anchor, idx) { return ({
                idx: idx,
                iou: ops_1.iou(new classes_1.BoundingBox(0, 0, anchor.x * const_1.CELL_SIZE, anchor.y * const_1.CELL_SIZE), new classes_1.BoundingBox(0, 0, width, height))
            }); })
    ...
    TinyYolov2LossFunction.prototype.createCoordAndScoreMasks = function () {
        var _this = this;
        return tf.tidy(function () {
            var coordBoxOffsetMask = tf.zeros([
                _this.numCells,
                _this.numCells,
                _this.numBoxes,
                _this.boxEncodingSize
            ]);
            var coordBoxSizeMask = tf.zeros([
                _this.numCells,
                _this.numCells,
                _this.numBoxes,
                _this.boxEncodingSize
            ]);
            var scoreMask = tf.zeros([
                _this.numCells,
                _this.numCells,
                _this.numBoxes,
                _this.boxEncodingSize
    ...
    TinyYolov2LossFunction.prototype.computeIous = function () {
        ...
        this.groundTruth.forEach(function (_a) {
            ...
            var boxIou = ops_1.iou(box.rescale(_this.reshapedImgDims), predBox.box.rescale(_this.reshapedImgDims));
